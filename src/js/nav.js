...
module.exports = function (document) {
  ...
  navEl = crel(
    ...
    crel(
      'a',
      { href: '.', class: !currentColorSet || currentColorSet === 'tech-brands' ? 'current' : '' },
      'Tech Brands'
    ),
    crel(
      'a',
      { href: '?nfl-teams', class: currentColorSet === 'nfl-teams' ? 'current' : '' },
      'NFL Teams'
    ),
    crel(
      'a',
      { href: '?nhl-teams', class: currentColorSet === 'nhl-teams' ? 'current' : '' },
      'NHL Teams'
    ),
    // crel('a', { href: 'http://twitter.com/collypops', target: '_blank', class: 'twitter' }, '@collypops'),
    // crel('a', {href: 'https://www.facebook.com/NameThatBlue', target: '_blank', 'class': 'facebook'}, 'FB.com/NameThatBlue'),
    crel(
      'a',
      { href: 'http://colin-gourlay.com/', target: '_blank', class: 'credit' },
      'Colin Gourlay ' + new Date().getFullYear()
    )
  );
  ...
  toggleEl.onclick = function toggle() {
    ...
    toggleEl.className =
      'nav_toggle' + (toggleEl.className.length > 14 ? '' : ' nav_toggle-active');
  };
